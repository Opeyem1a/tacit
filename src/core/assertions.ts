...
export function assertActionPriorityIsValid(
    ...
    if (
        ...
        throw new Error(
            'Action priority must be a positive integer less than 100'
        );
    }
}
...
function assertActionKeyDelayMsIsValid(
    ...
    if (!('delayMs' in action)) {
        throw new Error(`No 'delayMs' specified for ${action.kind} action`);
    }
...
function assertActionKeyCheckedIsValid(
    ...
    if (!('checked' in action)) {
        throw new Error(`No 'checked' specified for ${action.kind} action`);
    }
